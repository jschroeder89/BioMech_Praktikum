...
msg_t BluetoothWiimote::wiimoteReceive() {
  ...
  if (buffer[0] == 0xA1 && (buffer[1] == 0x37)){ //|| buffer[1] == 0x31)) {
      ...
      //buttons.sx	= (buffer[16]);
      //buttons.sy  	= (buffer[17]);
...
void BluetoothWiimote::bluetoothWiimoteDataBtn() {
  const uint8_t data[] = {0xA2, 0x12, 0x00, 0x30}; //0x30 0x37
...
void BluetoothWiimote::bluetoothWiimoteDataBtnAcc() {
  const uint8_t data[] = {0xA2, 0x12, 0x00, 0x31}; //0x31 //0x37
  wiimoteTransmit(data, 4);
}
